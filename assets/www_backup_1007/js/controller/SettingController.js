SettingController = Class.extend({
	...
	handlerClickDomain : function(row, param, element){
		...
		// o.id = 2 ;
		// o.name = "VP9 TV"
		// o.address = "http://tv.vp9.tv"
		// o.status = 1;
		// o.package = 2
		// o.services = [];
		// o.services.push({"id":1,"name":"Truyen hình","service":"tivi","status":1,"icon":"http://tv.vp9.tv/images/home/tivi.png"});
		// o.services.push({"id":2,"name":"Karaoke","service":"karaoke","status":1,"icon":"http://tv.vp9.tv/images/home/karaoke.png"});
		// o.services.push({"id":3,"name":"Film","service":"film","status":1,"icon":"http://tv.vp9.tv/images/home/movie.png"});
		// o.services.push({"id":4,"name":"Clip","service":"clip","status":1,"icon":"http://tv.vp9.tv/images/home/clip.png"});
		// o.services.push({"id":5,"name":"Music","service":"music","status":1,"icon":"http://tv.vp9.tv/images/home/music.png"});
		// o.services.push({"id":6,"name":"Game","service":"game","status":1,"icon":"http://tv.vp9.tv/images/home/game.png"});

		// o.packages = "<style>\#test {display: none}</style><div id=\"test\">123213213<\/div>";
		// o.message = "Ban chua dang ký gói cuoc nào!";
		// self.renderServer(o, "http://tv.vp9.tv");
		...
		var sendRequestToServer = function(addressURL){	
			...
			VP9.cordova.httpRequest(param, function(data) {
				...
				// if (!self.serverList[url].package) {
				// 	$('.stt_server .notify').html('Bạn chưa đăng ký gói cước nào!');
				// }
				// else {
				// 	var activeService = [];
				// 	if (self.serverList[url].services) {
				// 		$.each(self.serverList[url].services, function(k, service) {
				// 			activeService.push(service.id);
				// 		});
				// 	}
				// 	console.log(activeService, self.serverList, self.serverList[url].services);
				// 	self.renderServices(data, activeService);

				// 	activePackage = self.serverList[url].package;
				// }
				// self.renderPackages(data, activePackage);
				...
				self.readFile('server.' + url.replace('http://', ''), function(data) {
					try {
						...
						// 	var activeService = [];
						// 	if (self.serverList[url].services) {
						// 		$.each(self.serverList[url].services, function(service) {
						// 			activeService.push(service.id);
						// 		});
						// 	}
						// 	self.renderServices(data, activeService);
				...
				//$('.choose_server .add_server input').val('').prop('disabled', false);
			});
		}
	...
	renderServer : function(data, url) {
		...
		if (data.services) {
			...
			$('<div class="service_table"><div class="service_items col-sm-10"></div><div class="service_action col-sm-2"><a href="#" class="stt_submit" data-controller="SettingController" data-action="apply">Apply</a><a href="#" class="stt_reset" data-controller="SettingController" data-action="reset">Reset</a></div></div>').prependTo('.stt_server .stt_detail')
	...
	renderPackages : function(data, activePackage) {
		...
		var $price_table = $('<div class="price_table" data-url="' + data.address + '"><div class="col-sm-2 stt_submit"><a href="#" class="" data-controller="SettingController" data-action="submit">Apply</a></div><div class="price_attr col-sm-4"><ul></ul></div><div class="price_items col-sm-8"></div></div>').appendTo('.stt_server .stt_detail');
	...
	reset : function(){
		 var self = this;
        $.each($('.existed_server.active'), function(k, server) {
	...
	removeServer : function(row){
		...
		$('.stt_large_item ul li').first().trigger('click');
	},
...
/**/
		...
		$.each($('div.stt1 input[type="radio"]'), function(k, radio) {
		...
		var arrLanguage = [];
		$.each($('div.stt1 input[type="checkbox"]'), function(k, checkbox) {
