...
(function(cordova){
	...
	Simplefile.prototype.vp9odList = function(params, success, fail) {
		return cordova.exec( function(args) {
			success(args);
		}, function(args) {
			fail(args);
		}, 'Simplefile', 'vp9odList', [params]);
	};
    ...
    Wificontrols.prototype.getkbpersec = function(success, fail) {
        
        return cordova.exec( function(args) {
            success(args);
        }, function(args) {
            fail(args);
        }, 'Wificontrols', 'getkbpersec', []);
    };  
    ...
    Applist.prototype.generatelistappVP9 = function(params, success, fail) {
        console.log("generatelistappVP9");
        return cordova.exec( function(args) {
            success(args);
        }, function(args) {
            fail(args);
        }, 'Applist', 'generatelistappVP9', [params]);
    };      
    ...
    Wifi.prototype.getkbpersec = function(func) {
        
        window.plugins.wificontrols.getkbpersec( 
            function(returnVal) {if(typeof func == "function"){func(returnVal.returnVal);}
	...
	Apps.prototype.savelist = function(func) {
		window.plugins.applist.appList({}, 
			function(returnVal) {window.plugins.fileaccess.save(returnVal.returnVal, "/VP9Launcher/settings/applist.txt"); if(typeof func == "function"){func(returnVal.returnVal);}}, // Success function
			function(error) {alert('Generate App List Failed ' + error)}); // Failure function
	};
	...
	Apps.prototype.generateicons = function(func) {
		window.plugins.applist.generateIcons({}, 
			function(returnVal) {
                if(typeof func == "function"){alert("generateIcons func != null"); func(returnVal.returnVal);}else{
                    alert("generateIcons func == null");
                }
            }, // Success function
	...
	Apps.prototype.generatecss = function(func) {
		window.plugins.applist.appList({},
				function(returnVal) {
                        ...
                        // alert("CSS:" + JSON.stringify(appListArray));
						...
						for(var i = 0; i < appListArray.length; i++) {  
							...
							newStyle.innerHTML += '*[data-icons="'+filename+'"]{background-image:url('+iconPath+');}';
						}	
						document.getElementsByTagName('head')[0].appendChild(newStyle);
						var styleText = newStyle.innerHTML;
						
					window.plugins.simplefile.saveFile({content:styleText, file:"/VP9Launcher/settings/icons/icons.css"}, 			
					function(){},
					function(error) { window.plugins.fileaccess.save(styleText, "/VP9Launcher/settings/icons/icons.css"); }); 
    ...
    Apps.prototype.generatelistappVP9 = function(param, func, funcFail) {
        window.plugins.applist.generatelistappVP9(param, 
            function(returnVal) {

                if(typeof func == "function"){func(returnVal);}
            }, // Success function
            function(error) {
                        // alert('Generate App List Failed ' + error);
                        if(typeof funcFail == "function"){funcFail(error);}
                        }); // Failure function
    };
	...
	Launching.prototype.app = function(object, func) {
        ...
        // var prefix = activityChoice.substring(0,1);
        // if (prefix.indexOf(".") < 0) { // Neu k tim` thay "." thi` them "." len truoc
        //     activityChoice = "." + activityChoice;
        // }

		window.plugins.launch.app({package:packageChoice, activity:activityChoice}, 
    ...
    Launching.prototype.appVP9 = function(object, funcSuccess, funcFail) {
    ...
    var HandlerPlugin = function() {};
   
    HandlerPlugin.prototype.installapp = function(params, success, fail) {
        return cordova.exec( function(args) {
            success(args);
        }, function(args) {
            fail(args);
        }, 'HandlerEvents', 'install_app', [params]);
    };

    HandlerPlugin.prototype.settivichannel = function(params, success, fail) {
        return cordova.exec( function(args) {
            success(args);
        }, function(args) {
            fail(args);
        }, 'HandlerEvents', 'set_tivi_channel', [params]);
    };

    HandlerPlugin.prototype.canceltivichannel = function(params, success, fail) {
        return cordova.exec( function(args) {
            success(args);
        }, function(args) {
            fail(args);
        }, 'HandlerEvents', 'cancel_tivi_channel', [params]);
    };

    HandlerPlugin.prototype.playChannel = function(object, funcSuccess, funcFail) {
        var packageChoice = object['data-appPackage'];  //com.class.name
        var activityChoice = object['data-appActivity'] || "";  //.ActivityCall
        // alert("package: " + packageChoice + "\n activity: " + activityChoice);
	...
	//img_VP9Logo.png
	var VP9LSettings = function() {};
	
	VP9LSettings.prototype.vp9odlist = function(args) {
		window.plugins.simplefile.vp9odList({}, 
			function(returnVal) {
				if(args === "data"){
					vp9Callbacks.vp9odList(returnVal.returnVal);
				}else{
					...
					if(args  === "noStyle"){
						listWindow.innerHTML += '<div><div><img style="width:auto; height:100%;" src="file:///android_asset/www/img/img_vp9Logo.png" />VP9ods</div></div>';
						...
						listWindow.innerHTML += '<div><img id="vp9odlistLogo" style="width:auto; height:100%;" src="file:///android_asset/www/img/img_vp9Logo.png" /><div>VP9ods</div></div>';
					...
					document.getElementById('vp9odlistLogo').addEventListener("touchstart", function(){
						this.style.backgroundColor = "#356064";
						document.getElementById('vp9odlistLogo').removeEventListener("click");
						document.getElementById('themelistWindow')
						});
						
					document.getElementById('vp9odlistLogo').addEventListener("click", function(){
						this.style.backgroundColor = "transparent";
						document.getElementById('vp9odlistLogo').removeEventListener("touchstart");
						document.getElementById('vp9odlistLogo').removeEventListener("click");
					...
					for (var ii = 0; ii < final_appIntent.length; ii++)
					{
						final_appIntent[ii].addEventListener("click", function(){	
							...
							window.plugins.simplefile.openFile({file:'/VP9Launcher/settings/config.txt'}, 			
								function(returnVal){
									
									activeVp9od = JSON.parse(returnVal.returnVal);
									activeVp9od.active = themeID; 
									window.plugins.fileaccess.save(activeVp9od, "/VP9Launcher/settings/config.txt", window.plugins.fullscreen.restart); 
									
								},
								function(error) {alert('VP9LSettings Change Acive VP9od Error ' + error);}); 	
						}, false);
					}
				}
			...
			function(error) {alert('VP9LSettings VP9od List Error ' + error)}); // Failure function
	};
    ...
    cordova.addConstructor(function() {
		...
		window.plugins.fullscreencontrols = new Fullscreencontrols();
		...
		window.plugins.media = new Media();
		...
		window.plugins.vp9lsettings = new VP9LSettings();
		window.plugins.power = new Power();
		window.plugins.screenorient = new Screenorient();
        window.plugins.handlerplugin = new HandlerPlugin();
