...
$(window).keydown(function (e) {
    ...
    if (VP9.controlUI.getLayout() == "setting" && !VP9.userInput && (!event.altKey && !event.ctrlKey && keycode !== 37 && keycode !== 38 && keycode !== 39 && keycode !== 40 && keycode !== 13)) {
    	_controller.SettingControllBarGeneralController.handleInput(event);	
    	return;
    };
...
RemoteKey.Numpad = function(_e){
	var e = _e;
	if (VP9.userInput) {	
		console.log("RemoteKey.Numpad 1");
	}else{
		console.log("RemoteKey.Numpad 2");
...
RemoteKey.DownUpLeftRight = function(_e){
	...
	if (!VP9.userInput) {
		...
		if (layout == "notify") {
			_controller.NotifyView.keyDownUpLeftRight(e);
			return;
		}else if (layout == "update") {
			_controller.UpdateView.keyDownUpLeftRight(e);
			return;
		}
		...
		if (controller == undefined) {
			...
			if (layout=="setting") {
				controller = "SettingControllBarGeneralController";
...
RemoteKey.OK = function(_e){
	if (!VP9.userInput) {
		...
		if (layout == "notify") {
			_controller.NotifyView.keyEnter(e);
			return;
		}else if (layout == "update") {
			_controller.UpdateView.keyEnter(e);
			return;
		}
...
$(document).on('click', '.home_menu', function(){
	...
	if($('#setting-launcher').css("display") == "block"){
		...
		if (IsAndroid) {
			window.plugins.handlerplugin.canceltivichannel([], function(){	
			}, function(){
			});		
		};
		...
		setTimeout(function(){
			_controller.SettingControllBarGeneralController = new SettingControllBarGeneralController();	
		}, 500);
...
$(document).on('click', '.remove_cancel', function(){
	var eRemove = $(this);
	var rowParent = eRemove.parents('li.existed_server');
...
$(document).on('click', '.choose_server .add_server .btn_add', function(){
	if ($('.add_server input').val().trim() == "") {
		return;
	};

	$(this).removeClass('active');
	$('.stt_left li').removeClass('active')
...
$(document).on('focus', '.choose_server .add_server input', function(){
	...
	if (IsAndroid) {
		window.plugins.handlerplugin.canceltivichannel([], function(){
			console.log("canceltivichannel");
		}, function(){
			console.log("fail canceltivichannel");
		});		
	}
...
$(document).on('blur', '.choose_server .add_server input', function(){
	if (IsAndroid) {
		window.plugins.handlerplugin.settivichannel([], function(){
			console.log("settivichannel");
		}, function(){
			console.log("fail settivichannel");
		});
	}
	VP9.userInput = false;
	return false;
...
$(document).on('click', '.check_connection', function(){
    var controller = $(this).attr("data-controller");
    var action = $(this).attr("data-action");
...
$(document).on('focus', '#debug input', function(){
...
$(document).on('blur', '#debug input', function(){
	if (IsAndroid) {
		window.plugins.handlerplugin.settivichannel([], function(){
				console.log("settivichannel");
		}, function(){
