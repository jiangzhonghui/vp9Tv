...
VP9.socket = function(domain, socket) {
	...
	this.init = function(url) {
		...
		_this.ws.onerror = function() {
			...
			$('.stt_server .notify[data-domain="' + _this.domain + '"]').html('Kết nối với server thất bại!');
			_this.reconnect();
		}
...
VP9.login = function(socket) {
	...
	this.init = function() {
		...
		socket.on('close', function() {
			...
			$('.stt_server .stt_detail_head .room-code a[data-domain="' + _this.domain + '"]').show();
			_this.logged = false;
        	_this.token = '';
        	_this.code = '';
			_this.verify();
            ...
            try {
                var getAccountSuccess = function() {
                    ...
                    if (arrEmail.length > 0) {
						...
						$('.stt_server .notify[data-domain="' + _this.domain + '"]').html('Bạn cần đăng nhập tài khoản đi kèm với thiết bị!');
                ...
                var getAccountError = function() {
					$('.stt_server .notify[data-domain="' + _this.domain + '"]').html('Không lấy được thông tin tài khoản của thiết bị!');
    ...
    this.UIverify = function(data) {
        if (!data.email) {
            ...
            callbackError = function(data) {
				$('.stt_server .notify[data-domain="' + _this.domain + '"]').html('Không lấy được quyền đăng nhậpị!');
                console.log(data);
            }
	...
	this.UIlogin = function(data) {
		if (data.isLogin) {
		
		}
		else {
			$('.stt_server .notify[data-domain="' + _this.domain + '"]').html('Đăng nhập thất bại!');
		}
	}
    ...
    this.UIrelogin = function(data) {
        if (data.isSuccess) {
            ...
            $('.stt_server .stt_detail_head .room-code span[data-domain="' + _this.domain + '"]').html('<i class="glyphicon glyphicon-qrcode"></i> '  + data.code);
			$('.stt_server .stt_detail_head .room-code a[data-domain="' + _this.domain + '"]').hide();
...
VP9.remote = function(socket, login) {
	...
	socket.on('forward_message', function(data) {
		...
		switch (action) {
			...
			case 'close_app':
				...
				break;
			case 'run_app':        // send javascript cho tvapp
				_this.UIrunApp(data);
				break;
			case 'run_launcher':    // send javascript cho launcher
				_this.UIrunLauncher(data);
				break;
		}
	...
	this.UIopenApp = function(data) {
		// _this.controlApp('open_app', [{url: data.content.data}]);
		...
		var obj = data.content.data;
		cordova.exec(function(success) {
			_this.send(action, true);
		},
		function(error) {
			_this.send(action, false);
		}, 'RemotePlugin', "open_app", [obj]);
	...
	this.UIrunApp = function(data) {
		...
		function(error) {
			_this.send(action, false);
		}, 'RemotePlugin', "run_app", [{js: data.content.js}]);
	}

	this.UIrunLauncher = function(data){
		console.log("UIrunLauncher: " + data);
		cordova.exec(function(success) {
			_this.send(action, true);
		},
		function(error) {
			_this.send(action, false);
		}, 'RemotePlugin', "run_launcher", [{js: data.content.js}]);
	}
