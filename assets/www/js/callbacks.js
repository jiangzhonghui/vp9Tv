...
function vp9odLoaded(){
	...
	if (!navigator.onLine) {
        $('.home_notify').html('<span id="disconnect" style="color: red;">Không có kết nối mạng</span>');
    }
	document.addEventListener("online", function(){
		$('.home_notify').find('#disconnect').remove();
		VP9.isOnline = true;
		console.log("------------------ online");
...
VP9.init = function(){
	...
	$(document).on('click', '#btnWriteFile', function(){ 
	    ...
	    instanceHandleUpdate.writeLine(file, $('html').html(), function(data){
			alert("write ok");
			alert(data);
		}, function(fail){
			alert("write fail");
...
VP9.reloadicon = function(){
	...
	$.each(filmElems, function(k, filmElem) {
		...
		newImg.onerror = function() {
			img.css("background-image", 'url(img/logo_vp9.svg)');
 		}
		newImg.src = src;
